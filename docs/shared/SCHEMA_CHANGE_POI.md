...
20~30m 수준이어야 한다. ORS가 쓰는 SRTM 고도 데이터(30m 해상도)의 노이즈가
평지에서 누적되며 부풀려지는 알려진 특성이다.
...
스키마 변경은 필요 없다. 적재값을 그대로 저장하되 **강도 라벨 기준표에서 고도 임계값을
높게 잡고 거리를 주 기준으로 삼는 방향**으로 대응한다. 보정 계수는 여러 구간의 실측
비교가 필요해 이후 과제로 둔다.
