...
#### 보정 방법 — 크기 임계값이 아니라 경사 상한이다

흔히 쓰는 "N미터 미만 변화 무시" 방식은 **여기서 통하지 않는다.** 노이즈가 큰 값으로 오기 때문이다.
...
경사 6% 상한의 21.7m가 한강 평지의 실제 감각에 맞는다.

#### 대응 (W3)

스키마 변경은 필요 없다. ORS 값을 그대로 저장하되 **`intensity_level` 산출에서 고도를 빼고
거리만 쓴다.** 표시용 값(`total_ascent_m`)은 원값을 유지한다 — 우리가 보정한 숫자를
사용자에게 보여주려면 근거가 더 필요하다.

확장 지점은 **경사 상한 재계산기**로 둔다. 크기 임계값으로 만들어 두면 나중에 켜도
여전히 틀린 값이 나온다.

```
IntensityCalculator          거리만으로 판정 (현재)
  └ AscentEstimator          주입 가능
      ├ RawAscentEstimator          ORS 값 그대로 — 현재 기본값
      └ GradientCappedEstimator     좌표+고도로 재계산 — 켜면 판정에 투입 가능
```

`OrsRouteResult.geometryGeoJson`이 이미 3D 좌표를 담고 있어 **재계산 입력이 이미 확보돼 있다.**
별도 데이터 수집 없이 전환할 수 있다.

> 남은 검증 — 경사 상한값(6%인지 8%인지)은 이 구간 하나로 정할 수 없다.
> 여러 구간을 실측해 비교해야 한다. 고도를 판정에 다시 넣을 때 함께 정한다.
