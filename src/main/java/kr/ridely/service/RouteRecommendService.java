 ...
 *
 *   0. 저장된 응답 확인  Idempotency-Key가 왔고 창 안에 같은 키가 있으면 그것으로 끝난다
 ...
 *
 * <b>돌려줄지는 요청 조건이 아니라 키로 판정한다(0단계).</b> 조건으로 만들면 GPS 오차 때문에 같은 자리에서도 좌표가 흔들려 재시도가 새 요청으로 읽히고, 반대로 재추천은 조건이 같아 갇힌다. 저장 여부는 {@code RecommendationReplayStore}가 정한다 - 규칙으로 만든 응답과 회피에 실패한 응답은 남기지 않는다.
 *
 * 아직 없는 것
