...
The selector is overridable with `LLM_PROVIDER` (`google-genai` | `bedrock-converse` | `openai`). `openai` is the account admin's LiteLLM gateway (OpenAI-compatible; `OPENAI_BASE_URL` without `/v1`, `OPENAI_API_KEY`, `OPENAI_MODEL` = `bedrock-sonnet` etc.) — direct Bedrock permission is revoked on 2026-09-29, so prod must run with `LLM_PROVIDER=openai` after that. Never put the gateway address or key in the repo (it is public); they live in the EC2 `.env` only. The openai starter also drags in embedding/image/audio/moderation auto-configs that are on by default and demand a key, so `application.yml` sets those `spring.ai.model.*` to `none` — keep that when touching it. A developer's gitignored `application-local.yml` may hard-pin `spring.ai.model.chat` and beat the env var; override with `--spring.ai.model.chat=openai` when testing the gateway locally.
